...
grid.load();
grid.sortBy("ProductFieldsID", "desc");
//pager(grid);
...
function onRemove(e) {
    ...
    if (row.length > 1) {
        ...
        idlist = idlist.replace('NaN', row[0].ProductFieldsID);
        if (idlist != "") {
...
function onActionRenderer(e) {
    ...
    var s = '<a class="Edit_Button" href="FieldsList.aspx?id=' + _id + '" >管理</a>'
...
function editRow(row_uid) {
    var row = grid.getRowByUID(row_uid);
    if (row) {
        return "Modify.aspx?id=" + row.ProductFieldsID;
    }
    return "";
}
...
function editRowList(val) {
    ...
    var _href;
    if (row.length == 1) {
        _href = "Modify.aspx?id=" + row[0].ProductFieldsID;
    }
    else if (row.length > 1) {
        var idlist;
        for (var i = 0; i < row.length; i++) {
