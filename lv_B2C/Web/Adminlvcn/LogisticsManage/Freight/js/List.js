...
function GetCommany() {
    $.post('ajax/ajax.aspx?method=GetCommanyList', function (data) {
        Commanys = data;
    })
}
...
function onRemove(e) {
    ...
    if (row.length > 1) {
        ...
        for (var i = 0; i < row.length; i++) {
            idlist += row[i].LogisticsID;
            if (i < row.length - 1) {
                idlist += ",";
            }
        }
        idlist = idlist.replace('NaN', row[0].LogisticsID);
        if (idlist != "") {
            confirmDel(idlist, 'RemoveList');
        }
    }
    else if (row.length == 1) {
        confirmDel(row[0].LogisticsID, 'RemoveEmployees');
    }
    else {
        mini.alert('请选择商品');
    }
...
function delRow(row_uid) {
    ...
    if (row) {
        confirmDel(row.LogisticsID, 'RemoveEmployees');
    }
}
...
function editRow(row_uid) {
    var row = grid.getRowByUID(row_uid);
    if (row) {
        return "Modify.aspx?id=" + row.LogisticsID;
    }
    return "";
}
...
function editRowList(val) {
    ...
    var _href;
    if (row.length == 1) {
        _href = "Modify.aspx?id=" + row[0].LogisticsID;
    }
    else if (row.length > 1) {
        var idlist;
        for (var i = 0; i < row.length; i++) {
