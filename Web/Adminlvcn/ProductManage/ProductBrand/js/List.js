mini.parse();
var _ajaxPath = "/Adminlvcn/1ref/ajax/productbrand/";
var grid = mini.get("datagrid1");
var Genders = [{ id: 1, text: '是' }, { id: 0, text: '否'}]; //快速编辑时的数据
grid.load();
grid.sortBy("ProductBrandID", "desc");
//pager(grid);
...
//$(function () {
//    alert(123);
//    $("#seachText input").bind("keydown", function (e) {
//        if (e.keyCode == 13) {
//            search();
//            return false;
//        }
...
function onRemove(e) {
    ...
    if (row.length > 1) {
        ...
        for (var i = 0; i < row.length; i++) {
            idlist += row[i].ProductBrandID;
            if (i < row.length - 1) {
                idlist += ",";
            }
        }
        idlist = idlist.replace('NaN', row[0].ProductBrandID);
        if (idlist != "") {
            confirmDel(idlist, 'RemoveList');
        }
    }
    else if (row.length == 1) {
        confirmDel(row[0].ProductBrandID, 'RemoveEmployees');
    }
    else {
        mini.alert('请选择商品');
    }
...
function delRow(row_uid) {
    ...
    if (row) {
        confirmDel(row.ProductBrandID, 'RemoveEmployees');
    }
}
...
function editRow(row_uid) {
    var row = grid.getRowByUID(row_uid);
    if (row) {
        return "Modify.aspx?id=" + row.ProductBrandID;
    }
    return "";
}
...
function delData(_id, toMothod) {
    ...
    $.ajax({
        url: _ajaxPath + "productbrand.aspx?method=" + toMothod + "&id=" + _id,
        success: function (text) {
            mini.alert("删除成功");
            grid.reload();
        },
...
function updateRow(row_uid) {
    ...
    var json = mini.encode([rowData]);
    $.ajax({
        url: _ajaxPath + "productbrand.aspx?method=SaveEmployees",
        data: { employees: json },
        success: function (text) {
            grid.reload();
        },
