...
grid.load();
grid.sortBy("FieldsClassID", "desc");
//pager(grid);
...
function onRemove(e) {
    ...
    if (row.length > 1) {
        ...
        for (var i = 0; i < row.length; i++) {
            idlist += row[i].FieldsClassID;
            if (i < row.length - 1) {
                idlist += ",";
            }
        }
        idlist = idlist.replace('NaN', row[0].FieldsClassID);
        if (idlist != "") {
            confirmDel(idlist, 'RemoveList');
        }
    }
    else if (row.length == 1) {
        confirmDel(row[0].FieldsClassID, 'RemoveEmployees');
    }
    else {
        mini.alert('请选择商品');
    }
...
function delRow(row_uid) {
    ...
    if (row) {
        confirmDel(row.FieldsClassID, 'RemoveEmployees');
    }
}
...
function onActionRenderer(e) {
    ...
    var _id = record.FieldsClassID;

    var s = '<a class="Edit_Button" href="FieldsList.aspx?cid=' + _id + '" >管理</a>'
            +'<a class="Edit_Button" href="javascript:fEditRow(\'' + uid + '\')" target="_blank"><img title="快速编辑商品" src="/base_js/miniui/themes/icons/node.png"></a>'
...
function editRow(row_uid) {
    var row = grid.getRowByUID(row_uid);
    if (row) {
        return "Modify.aspx?id=" + row.FieldsClassID;
    }
    return "";
}
...
function editRowList(val) {
    ...
    var _href;
    if (row.length == 1) {
        _href = "Modify.aspx?id=" + row[0].FieldsClassID;
    }
    else if (row.length > 1) {
        var idlist;
        for (var i = 0; i < row.length; i++) {
