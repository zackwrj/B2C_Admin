...
var _ajaxUrl = "/Adminlvcn/1ref/ajax/productclass/";
...
function onEditNode() {
    ...
    if (node.length > 0) {
        ...
        edit_b.siblings('input.b_to_edit').focus().select().blur(function () {
            ...
            $.post(_ajaxUrl + 'productclass.aspx?method=UpdateField', { Action: "post", id: pcd, FName: 'Title', FVal: thisval }, function () { });
            $(this).remove();
        });
    }
    else {
    ...
    mini.confirm("确定删除分类？", "确定？",
        function (action) {
            if (action == "ok") {
                ...
                $.post(_ajaxUrl + "productclass.aspx?method=RemoveClass", { Action: "post", Pcd: pcd }, function (data) {
                    if (data > 0) {
...
$(function () {
    ...
    $up.click(function () {
        ...
        $.post(_ajaxUrl + 'productclass.aspx?method=UpdateField', { Action: "post", id: _thisPcd, FName: 'ShowOrder', FVal: _prevOrder }, function () {
            $.post(_ajaxUrl + 'productclass.aspx?method=UpdateField', { Action: "post", id: _prevPcd, FName: 'ShowOrder', FVal: _thisOrder }, function () {
                history.go(0);
            });
        });
    });
    ...
    $down.click(function () {
        ...
        $.post(_ajaxUrl + 'productclass.aspx?method=UpdateField', { Action: "post", id: _thisPcd, FName: 'ShowOrder', FVal: _nextOrder }, function () {
            $.post(_ajaxUrl + 'productclass.aspx?method=UpdateField', { Action: "post", id: _nextPcd, FName: 'ShowOrder', FVal: _thisOrder }, function () {
                history.go(0);
            });
        });
    });
